...
import { ICacheManagerPort, CacheSetOptions } from '../../application/ports/cache-manager-port.interface';

@Injectable()
export class RedisClientService implements ICacheManagerPort, OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(RedisClientService.name);
  private client: Redis | null = null;
  private isConnected = false;

  // In-memory fallback for environments without an active Redis instance
  private readonly fallbackStore = new Map<string, { value: any; expiresAt?: number }>();
  ...
  public onModuleInit(): void {
    ...
    try {
      this.client = new Redis({
        ...
        retryStrategy: (times) => {
          if (times > 3) {
            this.logger.warn(`Redis connection retries exceeded (${times}). Operating in resilient fallback mode.`);
      ...
      this.client.on('error', (err) => {
        ...
        this.logger.warn(`Redis client error: ${err.message}. Fallback cache active.`);
      });

      // Attempt async connection without blocking NestJS boot
      this.client.connect().catch((err) => {
        this.isConnected = false;
        this.logger.warn(`Initial Redis connection failed (${err.message}). Using resilient in-memory fallback store.`);
      });
    } catch (error: any) {
      this.logger.warn(`Could not initialize Redis client: ${error?.message || error}. Operating in fallback mode.`);
    }
  }
  ...
  public async delByPattern(pattern: string): Promise<number> {
    ...
    if (this.isRedisReady() && this.client) {
      try {
        ...
        this.logger.warn(`Redis DEL pattern failed for "${pattern}": ${err.message}`);
      }
    }
  ...
  public async invalidateByTag(tag: string): Promise<number> {
    ...
    if (this.isRedisReady() && this.client) {
      try {
        ...
        this.logger.warn(`Redis tag invalidation failed for tag "${tag}": ${err.message}`);
      }
    }
  ...
  public async flushAll(): Promise<void> {
    if (this.isRedisReady() && this.client) {
      try {
        ...
        this.logger.warn(`Redis FLUSHALL failed: ${err.message}`);
      }
    }
